...
int MajorVersion;								// Major version of Windows

// Used to disable 64bit redirection before searching a directory path. Not sure if this is still needed
...
DWORD BaseProcSpeed = 0;						// CPU clock speed 
DWORD ProcSpeed = 0;							// CPU clock speed adjusted for current CPU load
...
// Application folder and file paths
TCHAR AppFolderPath[MAX_PATH], AppPath[MAX_PATH], HelpPath[MAX_PATH], PluginsPath[MAX_PATH];
TCHAR tempFolderPath[MAX_PATH], tempRunCommand[MAX_PATH]; 

/** Settings variables **/

MailSettings *mail;								// Mail settings
GeneralSettings *settings;						// Other global settings

LinkedList llSessions = {0};					// List of session settings
int numSessions = 0;

LinkedList llGlobalGameSettings = {0};			// List of global game settings 
...
int FailedFetches = 0, FailedSends = 0;			// Track how many sends/receives have failed in a row

int NewCurrentPlayer = -1;
int YourPlayerNumber = 1;
int NumSessionWideChatEmails = 0;				// Used for counting incoming emails
int NumPrivateChatEmails = 0;

int selectedSession = -1;						// Session selected in the main listview
...
// Send and Receive email threads
...
// Original DialogProc backups (for overloading)
...
#include "callback_assigns.h"		// Defines which main app functions are available to plugins
